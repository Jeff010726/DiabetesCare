...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .filters label { margin: 0; font-size: 12px; color: #475467; }
    .filters input { min-width: 150px; background: white; }
    .hidden { display: none; }
...
<body>
    ...
    <main class="main">
      <div class="top">
        ...
        <div class="filters">
          ...
          <button type="button" class="primary" id="last-7" style="width:auto;margin:0;">7 days</button>
          <button type="button" class="primary" id="last-30" style="width:auto;margin:0;">30 days</button>
          <button type="button" class="primary" id="refresh" style="width:auto;margin:0;">Refresh</button>
        </div>
      </div>
      <section class="grid">
        <div class="card"><div class="muted">Visitors</div><div class="metric" id="metric-visitors">-</div></div>
        <div class="card"><div class="muted">Page Views</div><div class="metric" id="metric-pageviews">-</div></div>
        <div class="card"><div class="muted">Sessions</div><div class="metric" id="metric-sessions">-</div></div>
        <div class="card"><div class="muted">Booking Clicks</div><div class="metric" id="metric-bookings">-</div></div>
        <div class="card"><div class="muted">Contact Leads</div><div class="metric" id="metric-leads">-</div></div>
        <div class="card"><div class="muted">Registrations</div><div class="metric" id="metric-registrations">-</div></div>
      </section>
  ...
  <script>
    const state = { view: "dashboard", admin: null };
    ...
    async function loadAnalyticsMetrics() {
      const data = await api("/admin/api/analytics/dashboard" + rangeQuery());
      $("metric-visitors").textContent = data.metrics.visitors;
      $("metric-pageviews").textContent = data.metrics.pageViews;
      $("metric-sessions").textContent = data.metrics.sessions;
      $("metric-bookings").textContent = data.metrics.bookingClicks;
      $("metric-leads").textContent = data.metrics.leads;
      $("metric-registrations").textContent = data.metrics.registrations;
      return data;
    }
    ...
    function analyticsRows(data) {
      ...
      const add = (section, items) => {
        items.forEach((item) => rows.push([section, item.label || item.date || "-", item.count ?? item.pageViews ?? "-", item.visitors ?? "", item.bookingClicks ?? "", item.contactSubmits ?? ""]));
      };
      add("Top pages", data.topPages);
      add("Countries", data.topCountries);
      add("Referrers", data.topReferrers);
      add("Devices", data.topDevices);
      add("Daily trend", data.timeline);
    ...
    async function loadDashboard() {
      $("title").textContent = state.view === "traffic" ? "Traffic" : "Dashboard";
      const data = await loadAnalyticsMetrics();
      renderRows(["Section", "Label / Date", "Count / Page Views", "Visitors", "Bookings", "Contacts"], analyticsRows(data));
    }
